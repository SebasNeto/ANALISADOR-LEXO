...
int analex(FILE *input, char *lexema) {
    ...
    /*if (ch == '/' && ch_proximo == '/') {
		printf("Entrou em comentario \\");
		printf("Valor atual do caractere: %c", ch);
        while (ch != '\n') {
            ch = prox_char(input);
        }
    ...
    if (ch == '\\' && ch_proximo == '*') {
        ...
        while (!(ch == '*' && ch_proximo == '\\')) {
            ch = prox_char(input);
            ch_proximo = prox_char(input); // pegue o próximo caractere sem avançar o ponteiro
        }
        prox_char(input); // avance para o próximo caractere após o comentário
        return analex(input, lexema); // continue a análise sem retornar um token
    }
